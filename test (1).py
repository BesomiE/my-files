...
print("[DEBUG] Input dtype:", input_details[0]['dtype'])
print("[DEBUG] Model input shape:", input_details[0]['shape'])
...
try:
    while True:
        ...
        for i in range(count):
            if scores[i] > 0.7:
                x1 = max(0, int(boxes[i][1] * frame.shape[1]))
                y1 = max(0, int(boxes[i][0] * frame.shape[0]))
                x2 = min(frame.shape[1], int(boxes[i][3] * frame.shape[1]))
                y2 = min(frame.shape[0], int(boxes[i][2] * frame.shape[0]))
                detected_class_name = class_names[int(class_ids[i])]
                print(f"[DEBUG] Detected class index: {int(class_ids[i])}, name: {detected_class_name}, score: {scores[i]:.2f}") 
                ...
                if detected_class_name == "person":
                    ...
                    cv2.putText(frame, "Person ", (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    if (y2 - y1) > 100:
                        person_near = True
        ...
        if obstacle_mode:
            ...
            cv2.putText(frame, "Avoiding Obstacle", (30, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        ...
        # Priority 2: car very close
            ...
            continue  # go back to top to enter obstacle_mode
